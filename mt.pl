...
sub	type_field()
{
	...
	("select type_id, type_lib, type_cache "
	 "from mt_type "
	 "where type_ext like '%".$_[0]."|%';");
    return (@type_field);
}
...
sub	host_field()
{
	...
	("select host_id, host_url, host_lib, host_level "
	 "from mt_host "
	 "where host_id = '".$_[0]."';");
    return (@host_field);
}
...
sub	insert_url()
{
	...
	if ($_ =~ m/($prot):\/\/(?:www\.)?([-a-zA-Z0-9\.]*?\.*?[-a-zA-Z0-9]*?\.(?:[a-zA-Z]{2}?$ext_dom))(\/[-a-zA-Z0-9+\/%=~_\|!:,.;]*?[-a-zA-Z0-9+\/\%=~_\|]*?\.($ext_file.___))/im)
	{
		...
		$db->do("insert into mt_refer "
			"values (null, '".$host_refer."', '".$type_refer[0]."',"
			"'".$chemin_refer."', '', '', ".time().");");
	    }
	    else
